...
`multivariate-math/multivariate-math` has neither target. It already defines `CONTAINER_CMD` and `CONTAINER_NAME` (= `multivariate-math`),
...
- **Gitignore the artifacts:** add `$(CONTAINER_NAME)-*.tar` (i.e. `multivariate-math-*.tar`) — or just `*.tar` —
...
- **Nested podman:** `save`/`load` start no container, so they need no `--cgroups=disabled` and run
  fine nested.
- **`image-import` requires `FILE=`** — `make image-import FILE=multivariate-math-06-13-2026_12-00-00.tar`.
